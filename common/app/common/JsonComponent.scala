...
object JsonComponent extends Results with implicits.Requests {
  ...
  def apply(page: Page, html: Html)(implicit request: RequestHeader, env: Environment): RevalidatableResult = {
  ...
  def apply(page: Page, items: (String, Any)*)(implicit request: RequestHeader, env: Environment): RevalidatableResult = {
    val json = jsonFor(page, items: _*)
    resultFor(request, json)
  }

  private def jsonFor(page: Page, items: (String, Any)*)(implicit request: RequestHeader, env: Environment): String = {
